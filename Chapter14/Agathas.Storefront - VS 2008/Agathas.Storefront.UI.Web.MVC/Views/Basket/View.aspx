...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
    ...
    <script type="text/javascript">
        ...
        function removeItem(productId) {
            ...
            $.post('<%=Html.Resolve("/Basket/RemoveItem") %>', postData, 
                                                      updateBasket, "json");
        }
        ...
        function updateItemQtys() {
            ...
            $.post('<%=Html.Resolve("/Basket/UpdateItems") %>', jsonData, 
                                                     updateBasket, "json");
        }
        ...
        function updateShippingService(ddlShippingService) {
            ...
            $.post('<%=Html.Resolve("/Basket/UpdateShipping") %>', 
    ...
    <h2>Your Basket</h2>
     ...
     <table width="100%">
      ...
      <% foreach (BasketItemView item in Model.Basket.Items) { %>
         <tr>
            <td><%=Html.Encode(item.ProductName) %> - 
                <%=Html.Encode(item.ProductSizeName) %><br />
                <a href="JavaScript:removeItem(<%=Html.Encode(item.ProductId) %>)">
                                                                    remove this item</a>
            </td>
            <td><%=Html.TextBox("Qty-" + item.ProductId.ToString(), item.Qty, 
                                                 new { @class = "itemQtyBox" })%></td>
      ...
      <% } %> 
        ...
        <tr>            
            <td align="right"  colspan="3">Shipping:
                ...
                <% foreach (DeliveryOptionView deliveryOption in 
        ...
        <tr>            
            <td colspan="3"></td>            
            <td align="right">
               <%=Html.ActionLink("Check Out", "Checkout", "Checkout")%></td>
        </tr>
    </table>
    </div>
    <p></p>
	...
	{#foreach $T.Basket.Items as record}
	    <tr>
            ...
            <td><input class="itemQtyBox" id="Qty-{$T.record.ProductId}" 
                       type="text" value="{$T.record.Qty}" /></td>
        ...
        <tr>                        
            <td align="right"  colspan="3">Shipping:
                ...
                {#foreach $T.DeliveryOptions as deliveryOption}
                  ...
                  {#if $T.deliveryOption.Id == 
                                 $T.Basket.DeliveryOptionId} selected{#/if}

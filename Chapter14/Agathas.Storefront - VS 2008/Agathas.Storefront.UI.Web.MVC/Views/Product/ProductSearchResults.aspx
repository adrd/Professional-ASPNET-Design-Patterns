...
<h2><%= Html.ActionLink("Home", "Index", "Home")%> > 
    <%=Html.Encode(Model.SelectedCategoryName) %></h2>
</div>
...
<div style="margin-bottom: 41px;">
   ...
   <span id="numberOfProductsFound"><%=Html.Encode(Model.NumberOfTitlesFound) %></span>
   <span id="pageLinksTop">
   <%=Html.BuildPageLinksFrom(Model.CurrentPage, 
                              Model.TotalNumberOfPages, 
                              x => "JavaScript:displayPage("+ x +")")%></span>
</span>
</div>
   ...
   <% foreach (ProductSummaryView product in Model.Products){ %>      
      ...
      <img class="item-productimage" 
          ...
          <div class="item-productname">
            <%= Html.ActionLink(Html.Encode(product.BrandName) + " " + 
                Html.Encode(product.Name), "Detail", 
                                           "Product", 
                                           new { id = product.Id }, null)%></div>
 ...
 <script type="text/html" id="productItemTemplate">    
	...
	{#foreach $T.items as record}						                
        <li class="item-detail">
          ...
          <img class="item-productimage" 
              src="<%=Html.Resolve("/Content/images/Products/{$T.record.Id}.jpg") %>" 
          /></a>
           
          <div class="item-productname">
            <a href="<%=Html.Resolve("/Product/Detail/") %>
                {$T.record.Id}">{$T.record.BrandName} {$T.record.Name}</a>
          </div>
...
<div id="dialog-noproducts" title="No products found matching your refinement">
   <p>Your selection caused no results to be returned - 
      please widen your search  criteria.</p>
</div>     

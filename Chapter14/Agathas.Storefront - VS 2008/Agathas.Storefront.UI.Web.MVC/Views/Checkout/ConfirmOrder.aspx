<%@ Page Title="" Language="C#"
         ...
         Inherits="System.Web.Mvc.ViewPage<OrderConfirmationView>" %>
    ...
    <h2>Confirm Order</h2>
        ...
        %>
             <option value="<%=Html.Encode(deliveryAddress.Id)%>">
               <%=Html.Encode(deliveryAddress.Name)%></option>
        <%
              }%>
        </select>
        ...
        <% foreach(BasketItemView item in Model.Basket.Items) {%>
            <li><%=Html.Encode(item.Qty) %> of <%=Html.Encode(item.ProductName) %> at
                <%=Html.Encode(String.Format("{0:F}", item.ProductPrice))%></li>
        <% }%>
        </ul>

        <p>Total: <%= Html.Encode(String.Format("{0:F}", Model.Basket.ItemsTotal)) %></p>
        
        <p>DeliveryCharge: <%= Html.Encode(String.Format("{0:F}", 
                                            Model.Basket.DeliveryCost)) %></p>

        <p>Total: <%= Html.Encode(String.Format("{0:F}", 
                                  Model.Basket.BasketTotal )) %></p>

...
namespace ASPPatterns.Chap4.AnemicModel.Repository
{
    public class BankAccountRepository : IBankAccountRepository 
    {
        ...
        private void UpdateTransactionsFor(BankAccount bankAccount)
        {
            ...
            foreach (Transaction tran in bankAccount.Transactions)
            {
                using (SqlConnection connection =
                       new SqlConnection(_connectionString))
                {
                    ...
                    command.Parameters.Add(new SqlParameter("@Withdraw", tran.Withdraw));                    
        ...
        private IList<BankAccount> CreateListOfAccountsFrom(IDataReader datareader)
        {
            ...
            while (datareader.Read())
            {
                if (id != datareader["BankAccountId"].ToString())
                {
                    ...
                    bankAccount.AccountNo = new Guid(id);
                    bankAccount.Balance = Decimal.Parse(datareader["Balance"].ToString());
                    bankAccount.Transactions = transactions;
                    bankAccount.CustomerRef = datareader["CustomerRef"].ToString();                    
        ...
        private Transaction CreateTransactionFrom(IDataRecord rawData)
        {
            ...
            {
                Deposit = Decimal.Parse(rawData["Deposit"].ToString()),
                Withdraw = Decimal.Parse(rawData["Withdraw"].ToString()),
                Reference = rawData["Reference"].ToString(),
                Date = DateTime.Parse(rawData["Date"].ToString())
            };
        }
